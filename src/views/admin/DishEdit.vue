  ...
  <div>
    <div class="box" v-for="cate in backDishes" :key="cate.category.id">
      ...
      <ul>
        ...
        <li class="add" @click="showDialog({ category_id: cate.category.id })">
...
export default {
  ...
  methods: {
    getBackDish() {
      ...
      }).then((res) => (this.backDishes = res.data)); //数据请求成功后 存储
    },
    del(dish_id) {
